  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
      ...
      <section className={`relative min-h-screen overflow-hidden flex items-center justify-center ${isDark ? 'bg-gradient-to-br from-gray-800 via-blue-900 to-gray-900' : 'bg-gradient-to-br from-gray-100 via-blue-50 to-gray-300'}`}>
        ...
        <div className="absolute top-6 left-0 z-10 flex items-center justify-between w-full px-6">
      ...
      <section className={`py-24 ${isDark ? 'bg-gray-800' : 'bg-gray-50'}`}>
        <div className="container mx-auto px-4 max-w-[1280px]">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>精选内容与活动</h2>
              <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-500'}`}>正在进行</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className={`rounded-2xl overflow-hidden ${isDark ? 'bg-gray-700' : 'bg-white shadow-sm'}`}>
              <div className="h-48 bg-gradient-to-r from-black to-gray-800 relative">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-4xl mb-4">🏆</div>
                    <h3 className="text-white text-xl font-bold">那些值得被更多人看到的站点</h3>
                    <p className="text-gray-300 text-sm mt-2">按年份切换查看历届 HeoAwards 获奖网站与截图</p>
                  </div>
                </div>
              </div>
              <div className="p-4 flex justify-between items-center">
                <div className="flex items-center">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white mr-3">H</div>
                  <div>
                    <div className={`font-medium ${isDark ? 'text-white' : 'text-gray-800'}`}>HeoAwards 2025</div>
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>历年获奖站点专题页</div>
                  </div>
                </div>
                <button className={`px-4 py-1.5 rounded-full text-sm ${isDark ? 'bg-gray-600 text-white' : 'bg-gray-100 text-gray-800'}`}>查看</button>
              </div>
            </div>
            ...
            <div className={`rounded-2xl overflow-hidden ${isDark ? 'bg-gray-700' : 'bg-white shadow-sm'}`}>
              <div className="h-48 bg-gradient-to-r from-blue-500 to-purple-500 relative">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-4xl mb-4">📷</div>
                    <h3 className="text-white text-xl font-bold">批量处理图片、转格式</h3>
                    <p className="text-gray-300 text-sm mt-2">拖进自来就可以自动处理，支持多种格式和压缩选项</p>
                  </div>
                </div>
              </div>
              <div className="p-4 flex justify-between items-center">
                <div className="flex items-center">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 flex items-center justify-center text-white mr-3">洪</div>
                  <div>
                    <div className={`font-medium ${isDark ? 'text-white' : 'text-gray-800'}`}>配色网</div>
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>专业配色方案与色彩工具</div>
                  </div>
                </div>
                <button className={`px-4 py-1.5 rounded-full text-sm ${isDark ? 'bg-gray-600 text-white' : 'bg-gray-100 text-gray-800'}`}>探索</button>
              </div>
            </div>
          </div>
      ...
      {/* Blog Section - 洪给星空 */}
      <section className="py-24 bg-gradient-to-b from-blue-900 to-blue-950 text-white">
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-white text-gray-800 hover:bg-gray-100 shadow-lg'} ${showBackToTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
        aria-label="返回顶部"
      >
        ↑
      </button>
